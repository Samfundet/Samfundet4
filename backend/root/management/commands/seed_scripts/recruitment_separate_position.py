    ...
    for recruitment in recruitments:
        for i in range(5):
            RecruitmentSeparatePosition.objects.create(
                recruitment=recruitment, name_nb=f'{words(2)} ({i})', name_en=f'{words(2)} ({i})', description_en=f'{words(5)}', description_nb=f'{words(5)}', url='https://open.spotify.com/track/6rbLnlNqSDSlvw5Z5beetG'
            )
            created_count += 1
