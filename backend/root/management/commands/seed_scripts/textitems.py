...
def seed():
    text_items = [
        ...
        },
        {
            ...
            'text_en':
                'It is important that you register so that nobody can abuse your membership number, and so that you can replace a lost card. You can register at out member database medlem.samfundet.no. The card is valid without a picture one week after it is purchased. After one week, you must have a photo of yourself on the card for it to be valid. Remember that you must register the first time you receive a card. When you later get a new sticker or a new card, you only need to update the information already saved, you do not need to register all over again.'
        },
        {
