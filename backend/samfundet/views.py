...
class RecruitmentAdmissionSetInterviewView(APIView):
    permission_classes = [IsAuthenticated]
    serializer_class = InterviewSerializer

    def put(self, request: Request, pk: str) -> Response:
        admission = get_object_or_404(RecruitmentAdmission, id=pk)
        data = request.data.dict() if isinstance(request.data, QueryDict) else request.data
        serializer = self.serializer_class(data=data)
        if serializer.is_valid():
            existing_interview = admission.interview
            if existing_interview:
                ...
                admission_serializer = RecruitmentAdmissionForGangSerializer(RecruitmentAdmission.objects.get(id=pk))
                return Response(admission_serializer.data, status=status.HTTP_200_OK)

            new_interview = serializer.save()
            admission.interview = new_interview
            admission.save()
            admission_serializer = RecruitmentAdmissionForGangSerializer(RecruitmentAdmission.objects.get(id=pk))
            return Response(admission_serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
