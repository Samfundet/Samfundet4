...
class RecruitmentApplicationSetInterviewView(APIView):
    ...
    def put(self, request: Request, pk: str) -> Response:
        admission = get_object_or_404(RecruitmentApplication, id=pk)
        data = request.data.dict() if isinstance(request.data, QueryDict) else request.data
        ...
        if serializer.is_valid():
            ...
            if existing_interview:
                ...
                admission_serializer = RecruitmentApplicationForGangSerializer(RecruitmentApplication.objects.get(id=pk))
