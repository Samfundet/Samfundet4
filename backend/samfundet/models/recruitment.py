...
class RecruitmentAdmission(CustomBaseModel):
    ...
    def save(self, *args: tuple, **kwargs: dict) -> None:
        ...
        if not self.applicant_priority:
            current_applications_count = RecruitmentAdmission.objects.filter(
...
class AdmissionFileUpload(CustomBaseModel):

    admission_id = models.ForeignKey(RecruitmentAdmission,
                                     on_delete=models.CASCADE)

    admission_image = models.ImageField(help_text='Applicant image',
                                        upload_to=upload_file_recruitment_path,
                                        blank=True,
                                        null=True,
                                        verbose_name='Applicant image')

    admission_video = models.FileField(help_text='Applicant file',
                                       blank=True,
                                       upload_to=upload_file_recruitment_path,
                                       null=True,
