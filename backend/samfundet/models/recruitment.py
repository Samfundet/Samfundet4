...
from samfundet.utils import upload_image_reqruitment_path
    ...
    def clean(self, *args: tuple, **kwargs: dict) -> None:
        ...
        if self.actual_application_deadline < self.visible_from:
            raise ValidationError(
                'Visible from should be before application deadline')

        if self.actual_application_deadline < self.shown_application_deadline:
            raise ValidationError(
                'Shown application deadline should be before the actual application deadline'
            )

        if self.reprioritization_deadline_for_applicant < self.actual_application_deadline:
            raise ValidationError(
                'Actual application deadline should be before reprioritization deadline for applicants'
            )
...
class RecruitmentAdmission(CustomBaseModel):
    ...
    def clean_image_upload(self):
        ...
        if image.size > 25 * 1024 * 1024:  #  limit file size 25MB
            raise ValidationError("Image file too large ( > 10mb )")

        if not magic.from_buffer(image.read(2048),
                                 mime=True).startswith('image/'):
            raise ValidationError("Invalid image format.")
        return image

    def clean_file_upload(self):
        file = self.clean_data.get('file_upload', False)
        if file:
            if file.size > 200 * 1024 * 1024:  #  limit file size to 200MB
                raise ValidationError("File to large (> 20mb )")
        return file

    # TODO: Make secure
    image_upload = models.ImageField(help_text='Applicant image',
                                     upload_to=upload_image_reqruitment_path,
                                     blank=True,
                                     null=True,
                                     verbose_name='Applicant image')
    # TODO: make secure
    file_upload = models.FileField(help_text='Applicant file',
                                   blank=True,
                                   upload_to=upload_image_reqruitment_path,
                                   null=True,
                                   verbose_name='Applicant file')
    admission_text = models.TextField(
        help_text='Admission text for the admission')
    ...
    user = models.ForeignKey(User,
                             ...
                             related_name='admissions')
    applicant_priority = models.IntegerField(
    ...
    def save(self, *args: tuple, **kwargs: dict) -> None:
        ...
        if not self.interview:
            ...
            shared_interview = RecruitmentAdmission.objects.filter(
                user=self.user,
