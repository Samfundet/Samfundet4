...
class TestRecruitmentClean:
    ...
    def test_visible_from_before_application_deadline(self, fixture_org):
        ...
        with pytest.raises(ValidationError) as error:
            _create_recruitment_with_dt(
                overrides={'visible_from': future_more})
        assert 'Shown application deadline should be after visible' in dict(
            error.value)['actual_application_deadline']
        assert 'Visible from should be before shown application deadline' in dict(
            error.value)['visible_from']
    ...
    def test_application_deadline_before_reprioritization_deadline(
        ...
        with pytest.raises(ValidationError) as error:
            _create_recruitment_with_dt(
                overrides={'actual_application_deadline': future_more})

        assert 'Actual application deadline should be before reprioritization deadline for applicants' in dict(
            error.value)['actual_application_deadline']
        assert 'Reprioritization application deadline should be after actual deadline for applicants' in dict(
            error.value)['reprioritization_deadline_for_applicant']

    def test_reprioritization_deadline_for_applicant_before_reprioritization_deadline_for_groups(
            self, fixture_org):
        future_more = timezone.now() + timezone.timedelta(days=FUTURE_DAYS + 2)
        with pytest.raises(ValidationError) as error:
            _create_recruitment_with_dt(
                overrides={
                    'reprioritization_deadline_for_applicant': future_more
                })
        assert 'Reprioritization deadline for groups should be after reprioritization deadline for groups' in dict(
            error.value)['reprioritization_deadline_for_groups']
    ...
    def test_actual_deadline_before_shown_deadline(self, fixture_org):
        ...
        with pytest.raises(ValidationError) as error:
            _create_recruitment_with_dt(
                overrides={'shown_application_deadline': future_more})
        assert 'Actual application deadline should be after the shown application deadline' in dict(
            error.value)['actual_application_deadline']
        assert 'Shown application deadline should be before the actual application deadline' in dict(
            error.value)['shown_application_deadline']
