...
class MenuItemReadSerializer(serializers.ModelSerializer):
    food_preferences = FoodPreferenceSerializer(many=True, read_only=True)
...
class MenuSerializer(serializers.ModelSerializer):
    menu_items = MenuItemReadSerializer(many=True)
...
class RecruitmentPositionSerializer(serializers.ModelSerializer):
    ...
    def _update_interviewers(self, recruitment_position: RecruitmentPosition,
                             interviewer_objects: list[dict]) -> None:
        try:
            interviewers = []
