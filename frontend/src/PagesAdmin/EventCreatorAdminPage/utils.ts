...
import { FormType } from "./hooks/useEventCreatorForm";
...
export function mapEventToFormValues(params: {
    ...
    return {
      ...
      visibility_from_dt: forTemplate ? '' : event.visibility_from_dt ? utcTimestampToLocal(event.visibility_from_dt, false) : '',
      visibility_to_dt: forTemplate ? '' : event.visibility_to_dt ? utcTimestampToLocal(event.visibility_to_dt, false) : '',
    };
}
