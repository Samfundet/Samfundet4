...
import {
  ...
  getBilligCheckoutTickets,
  getBilligPurchaseFailure,
} from '~/apis/billig/billigApi';
                ...
                {ticketGroup.price_groups.map((priceGroup) => (
                  <label key={priceGroup.id} className={styles.formRow}>
                    ...
                    <select name={`price_${priceGroup.id}_count`} defaultValue="0" className={styles.select}>
                      {[...Array(ticketGroup.per_price_group_limit + 1).keys()].map((count) => (
...
export function BilligPurchaseFailurePage() {
  ...
  const billigEventId = retryEvent?.billig?.id;
  const {
    data: retryTicketGroups,
    ...
    enabled: billigEventId !== undefined,
    retry: false,
  });
  const retryTicketStatus = isRetryError
    ? t(KEY.ticket_checkout_load_error)
    : isRetryPending
    ...
    <Page className={styles.page} loading={isFailureLoading}>
      <div className={styles.shell}>
        ...
        {shouldUseSignedMode && storedContext && (
          <>
            {retryTicketGroups && retryTicketGroups.length > 0 ? (
              <BilligSignedFailureForm
                context={storedContext}
                ticketGroups={retryTicketGroups}
                onParsedValues={setSignedRetryValues}
                t={t}
              />
            ) : (
              <section className={styles.panel}>{retryTicketStatus}</section>
            )}
            {signedRetryValues && retryTicketGroups && retryTicketGroups.length > 0 && (
              <section className={`${styles.panel} hidden-if-error`}>
                ...
                <BuyTicketForm
                  ...
                  ticketGroups={retryTicketGroups}
                  initialValues={signedRetryValues}
                />
              </section>
            )}
          </>
        ...
        {!shouldUseSignedMode && failureData?.found && failureData.retry_possible && retryEvent && (
          <section className={styles.panel}>
            ...
            {retryTicketGroups && retryTicketGroups.length > 0 ? (
              <BuyTicketForm event={retryEvent} ticketGroups={retryTicketGroups} initialValues={dbRetryInitialValues} />
            ) : (
              <p role="status">{retryTicketStatus}</p>
            )}
          </section>
        )}
